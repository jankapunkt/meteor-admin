...
AutoForm.hooks({
  admin_insert: {
    onSubmit (insertDoc, updateDoc, currentDoc) {
      const hook = this
      Meteor.call('adminInsertDoc', insertDoc, AdminCache.get('admin_collection_name'), function (e, r) {
        if (e) {
          return hook.done(e)
        } else {
          return adminCallback('onInsert', [AdminCache.get('admin_collection_name', insertDoc, updateDoc, currentDoc)], collection => hook.done(null, collection))
        }
      })
      return false
    },
  ...
  admin_update: {
    onSubmit (insertDoc, updateDoc, currentDoc) {
      const hook = this
      Meteor.call('adminUpdateDoc', updateDoc, AdminCache.get('admin_collection_name'), AdminCache.get('admin_id'), function (e, r) {
        if (e) {
          return hook.done(e)
        } else {
          return adminCallback('onUpdate', [AdminCache.get('admin_collection_name', insertDoc, updateDoc, currentDoc)], collection => hook.done(null, collection))
        }
      })
      return false
    },
  ...
  adminUpdateUser: {
    onSubmit (insertDoc, updateDoc, currentDoc) {
      Meteor.call('adminUpdateUser', updateDoc, AdminCache.get('admin_id'), this.done)
      return false
    },
    onSuccess (formType, result) {
