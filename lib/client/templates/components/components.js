...
Template.AdminDashboardView.helpers({
  hasDocuments () {
    return __guard__(AdminCollectionsCount.findOne({ collection: AdminCache.get('admin_collection_name') }), x => x.count) > 0
  },
  newPath () {
    return Router.path(`adminDashboard${AdminCache.get('admin_collection_name')}New`)
  }
})

Template.adminEditBtn.helpers({
  path () {
    return Router.path(`adminDashboard${AdminCache.get('admin_collection_name')}Edit`, { _id: this._id })
  }
})
